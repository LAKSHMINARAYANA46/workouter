...
class Workout {
  id = Date.now() + ''.slice(-9); //str
  date = new Date(); //date obj
  #clicks = 0;
...
class App {
  ...
  _loadMap(postion) {
    ...
    //render the marker once the map is loaded
    this.#workouts.forEach(wrk => {
      this._renderMarker(wrk);
    });
    //map clicks event handler
  ...
  _newWorkout(e) {
    ...
    //validate the input
    const isNumInputs = (...inputs) => inputs.every(ip => Number.isFinite(ip));
    const positiveInput = (...inputs) => inputs.every(ip => ip > 0);
    ...
    //clear form  fields & leave the focus
    this._clearForm();
  }
  ...
  _moveToWorkout(e) {
    if (!this.#map) return; //prevents error when map obj is not loaded
